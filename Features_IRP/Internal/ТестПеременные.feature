...
Сценарий: локальные переменные
	// при записи в перемменныю с указанием $$  значение переменной при переоткрытии она не изменится ЛУЧШЕ не использовать!
	// И я запоминаю заголовок текущего окна как "$ЗаголовокОкнаПоступлениеТоваров$"
	// при записи в перемменныю <БЕЗ> указания $$  значение переменной при переоткрытии она будет соответственно измениться 
	И я запоминаю заголовок текущего окна как "ЗаголовокОкнаПоступлениеТоваров"
...
Сценарий: глобальные переменные	
	// значение глобальной переменной не изменяемо! перезапись не поможет, надо идти через шаг удаления переменной
	// или используя $$$$
	// !!!рекомендуется все-таки использовать шаг на удаление переменной!!!!!
