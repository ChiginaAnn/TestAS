...
чтобы проанализировать разницу между тем что было заказано и что было факстически доставлено 
...
Сценарий: _0402 проверка заполнения цены из соглашения
	...
	* Изменения вида цен и проверка его перезаполнения при перевыборе соглашения
		// И в таблице 'ItemList' я активизирую поле "Вид цены" - убираем для повышения читабельности сценария
